...
public class Test_A04_CharImage {
    ...
    public Object[][] mapGen() {
        ...
        for(int rows = 1; rows <= numRows; rows++) {
            for(int cols = 1; cols <= numCols; cols++) {
                maps[index][0] = new CharImage(rows, cols, defaultChar);
                maps[index][1] = rows;
                maps[index][2] = cols;
    ...
    public void test_getRowCnt(CharImage myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        Assert.assertEquals(myMap.getRowCnt(), rowCnt.intValue());        
    }

    @Test(dataProvider = "mapGen")
    public void test_getColCnt(CharImage myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        Assert.assertEquals(myMap.getColCnt(), colCnt.intValue());        
    }

    @Test(dataProvider = "mapGen")
    public void test_isValidPosition(CharImage myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
        // Check negative
    ...
    @Test(dataProvider = "mapGen")
    public void test_GetAndSetPos(CharImage myMap, Integer rowCnt, Integer colCnt, Character defaultChar) {
    ...
    public void test_toString() {
        CharImage m = new CharImage(5, 10, 'a');
        Assert.assertEquals(m.toString(), "5 x 10 CharImage (default: a)");

        m = new CharImage(7, 8, '@');
        Assert.assertEquals(m.toString(), "7 x 8 CharImage (default: @)");
    }
